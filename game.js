...
// ── Pixel art pipeline (1280×720 high-res, 1× upscale) ─────────────
const PX_W = 1280, PX_H = 720, PX_SCALE = 1;
...
// 24-color retro palette
const P = {
  BLACK:  "#0a0a10", WHITE:  "#f0f0e8",
  DGRAY:  "#1a1a22", MGRAY:  "#3a3a44", LGRAY:  "#6a6a74",
  DBROWN: "#2a1a0a", MBROWN: "#5a3a1a", LBROWN: "#8a6a3a",
  WOOD:   "#4a2a10", FLOOR:  "#1a1818",
  DRED:   "#4a1020", RED:    "#8a2040", LRED:   "#c04060",
  DBLUE:  "#0a1a3a", BLUE:   "#1a3a6a", LBLUE:  "#3a5aaa",
  DGREEN: "#0a2a1a", GREEN:  "#1a5a3a", LGREEN: "#3a8a5a",
  GOLD:   "#8a6a20", AMBER:  "#6a4a10", CYAN:   "#2a6a6a",
};
// Pixel drawing helpers
function px(r, g, b) { return "rgb("+Math.floor(r)+","+Math.floor(g)+","+Math.floor(b)+")"; }
function pRect(x, y, w, h, c) { pxCtx.fillStyle = c; pxCtx.fillRect(x, y, w, h); }
function pStroke(x, y, w, h, c, lw) {
  pxCtx.strokeStyle = c; pxCtx.lineWidth = lw || 1; pxCtx.strokeRect(x, y, w, h);
}
function pHRect(x, y, w, h, r, c) { // rounded rect at pixel scale
  pxCtx.fillStyle = c;
  const rr = Math.min(r, w/2, h/2);
  pxCtx.beginPath(); pxCtx.moveTo(x+rr, y);
  pxCtx.lineTo(x+w-rr, y); pxCtx.quadraticCurveTo(x+w, y, x+w, y+rr);
  pxCtx.lineTo(x+w, y+h-rr); pxCtx.quadraticCurveTo(x+w, y+h, x+w-rr, y+h);
  pxCtx.lineTo(x+rr, y+h); pxCtx.quadraticCurveTo(x, y+h, x, y+h-rr);
  pxCtx.lineTo(x, y+rr); pxCtx.quadraticCurveTo(x, y, x+rr, y);
  pxCtx.closePath(); pxCtx.fill();
}
function pHStroke(x, y, w, h, r, c, lw) {
  pxCtx.strokeStyle = c; pxCtx.lineWidth = lw || 1;
  const rr = Math.min(r, w/2, h/2);
  pxCtx.beginPath(); pxCtx.moveTo(x+rr, y);
  pxCtx.lineTo(x+w-rr, y); pxCtx.quadraticCurveTo(x+w, y, x+w, y+rr);
  pxCtx.lineTo(x+w, y+h-rr); pxCtx.quadraticCurveTo(x+w, y+h, x+w-rr, y+h);
  pxCtx.lineTo(x+rr, y+h); pxCtx.quadraticCurveTo(x, y+h, x, y+h-rr);
  pxCtx.lineTo(x, y+rr); pxCtx.quadraticCurveTo(x, y, x+rr, y);
  pxCtx.closePath(); pxCtx.stroke();
}
function pCirc(x, y, r, c) { pxCtx.fillStyle = c; pxCtx.beginPath(); pxCtx.arc(x, y, r, 0, Math.PI*2); pxCtx.fill(); }
function pSCirc(x, y, r, c, lw) { pxCtx.strokeStyle = c; pxCtx.lineWidth = lw || 1; pxCtx.beginPath(); pxCtx.arc(x, y, r, 0, Math.PI*2); pxCtx.stroke(); }
...
const lobbyNpcs = [
  ...
  { x: 490, y: 162, size: 16, vx: 0.12, vy: 0.08, moodTime: 0, color: "#eeeef8", skin: "#f7ddc2", role: "staff", anchor: { x: 490, y: 162 }, roamRadius: 52, type: "female" },
  { x: 640, y: 162, size: 16, vx: -0.12, vy: 0.08, moodTime: 0, color: "#eeeef8", skin: "#dba88a", role: "staff", anchor: { x: 640, y: 162 }, roamRadius: 52, type: "male" },
  { x: 800, y: 162, size: 16, vx: 0.14, vy: 0.1,  moodTime: 0, color: "#eeeef8", skin: "#c47a52", role: "staff", anchor: { x: 800, y: 162 }, roamRadius: 52, type: "female" },
  // Concierge near desk
  { x: 1090, y: 185, size: 16, vx: 0.18, vy: 0.14, moodTime: 0, color: "#d4a84a", skin: "#f7ddc2", role: "staff", anchor: { x: 1090, y: 185 }, roamRadius: 42, type: "male" },
  // Guests walking around (adjusted for no fountain)
  { x: 400, y: 400, size: 16, vx: 0.3,  vy: 0.22, moodTime: 0, color: "#ff9dac", skin: "#f7ddc2", role: "customer", type: "female" },
  { x: 660, y: 525, size: 16, vx: -0.28, vy: 0.2, moodTime: 0, color: "#95d8ff", skin: "#dba88a", role: "customer", type: "male" },
  { x: 880, y: 445, size: 16, vx: 0.22, vy: -0.28, moodTime: 0, color: "#dcb2ff", skin: "#c47a52", role: "customer", type: "female" },
  { x: 450, y: 595, size: 16, vx: 0.28, vy: -0.2, moodTime: 0, color: "#f7cb6f", skin: "#dba88a", role: "customer", type: "male" },
  { x: 785, y: 315, size: 14, vx: 0.2,  vy: 0.28, moodTime: 0, color: "#bde3a6", skin: "#8a5a38", role: "customer", type: "child" },
  { x: 1025, y: 580, size: 16, vx: -0.25, vy: -0.2, moodTime: 0, color: "#ffb8c0", skin: "#f7ddc2", role: "customer", type: "female" }
];
...
const backstageNpcs = [
  ...
  // Crew in break area
  { x: 185, y: 435, size: 16, vx: 0.22, vy: 0.18, moodTime: 0, color: "#889099", skin: "#f7ddc2", role: "staff", anchor: { x: 185, y: 435 }, roamRadius: 72, type: "male" },
  { x: 270, y: 460, size: 16, vx: -0.18, vy: 0.22, moodTime: 0, color: "#667788", skin: "#dba88a", role: "staff", anchor: { x: 270, y: 460 }, roamRadius: 72, type: "male" }
];
// ── Floor room data ───────────────────────────────────────────────────────────
...
function drawHumanoid(x, y, size, bodyColor, skinColor, legColor, isStaff, type) {
  type = type || "male";
  const cx = x + size / 2;
  const s = size / 16;

  // Proportion overrides by type
  let headR, headCY, torsoW, torsoH, torsoY, armW, armH, armY, legW, legH, legY, legGap;

  if (type === "child") {
    // Child: shorter, bigger head, squatter torso
    headR = 7 * s;
    headCY = y + headR + s;
    torsoW = 8 * s;
    torsoH = 8 * s;
    torsoY = headCY + headR + s;
    armW = 2.5 * s;
    armH = 6 * s;
    armY = torsoY + 0.5 * s;
    legW = 3 * s;
  ...
  // Female: hair (simple arc on top)
  if (type === "female") {
    fillRoundedRect(cx - headR*0.8, headCY - headR*1.1, headR*1.6, headR*0.8, headR*0.4, "rgba(0,0,0,0.2)");
  }
}
...
function drawLobby() {
  ...
  for (const npc of lobbyNpcs) {
    ...
    drawHumanoid(npc.x, npc.y, npc.size, bodyColor, npc.skin || "#f7ddc2", legColor, isStaff, npc.type || "male");
  }
}
...
function drawFloor7() {
  ...
  for (let r=0;r<48;r++)for(let c=0;c<80;c++)if((r+c)%2===0)pRect(c*16,r*16,16,16,"#70b8d8");
  pxCtx.restore();

  // Pool basin
  const px=180,py=220,pw=960,ph=420;
  pHRect(px,py,pw,ph,36,"#060e14");
  pHStroke(px,py,pw,ph,36,"#3ab8e8",5);
  // Deck tiles
  pHRect(px,py,pw,24,18,"#0e1a20");
  pHStroke(px,py,pw,24,18,"#5ac8f0",2);
  // Water fill with gradient
  const wg=pxCtx.createLinearGradient(0,py+24,0,py+ph);
  wg.addColorStop(0,"#0a2a38");wg.addColorStop(0.5,"#0c3448");wg.addColorStop(1,"#041820");
  pHRect(px+16,py+24,pw-32,ph-40,28,wg);
  // Water surface ripples
  pxCtx.save();pxCtx.globalAlpha=0.06;
  for(let s=0;s<36;s++){const sx=px+32+s*28,sy=py+36+(s%7)*20;pRect(sx,sy,28,8,"#90e8ff");}
  pxCtx.restore();
  // Lane lines
  pxCtx.save();pxCtx.strokeStyle="#70d0e8";pxCtx.lineWidth=2;pxCtx.setLineDash([20,30]);
  for(let l=0;l<5;l++){pxCtx.beginPath();pxCtx.moveTo(px+28,py+68+l*68);pxCtx.lineTo(px+pw-28,py+68+l*68);pxCtx.stroke();}
  pxCtx.setLineDash([]);pxCtx.restore();
  // 2 Pool ladders
  for(let ld=0;ld<2;ld++){const lx=ld?px+pw-24:px+20;pxCtx.strokeStyle="#6688aa";pxCtx.lineWidth=4;
    pxCtx.beginPath();pxCtx.moveTo(lx,py+32);pxCtx.lineTo(lx,py+160);pxCtx.stroke();
    for(let r=0;r<5;r++){const ry=py+40+r*28;pxCtx.beginPath();pxCtx.moveTo(lx,ry);pxCtx.lineTo(lx+(ld?-16:16),ry);pxCtx.stroke();}}
  // Depth markers
  pxCtx.fillStyle="#7799aa";pxCtx.font="bold 10px monospace";pxCtx.textAlign="right";pxCtx.fillText("1.2m",px+32,py+232);
  pxCtx.textAlign="left";pxCtx.fillText("2.5m",px+pw-32,py+288);

  // 3 Changing rooms
  for(let cr=0;cr<3;cr++){const cy=40+cr*190;
    pHRect(22,cy,146,168,12,"#0e1418");pHStroke(22,cy,146,168,12,"#88aacc",3);
    // Louver door
    pHRect(38,cy+100,34,56,8,"#182028");pHStroke(38,cy+100,34,56,8,"#5088b8",2);
    // Door handle
    pCirc(62,cy+128,4,"#d0c090");
    // Door slats
    for(let sl=0;sl<4;sl++)pHRect(41,cy+106+sl*14,28,6,3,"#284058");
    // Room number plate
    pHRect(55,cy+12,30,18,4,"#c0a870");pxCtx.fillStyle="#0e1418";pxCtx.font="bold 11px monospace";pxCtx.textAlign="center";pxCtx.fillText(String(cr+1),70,cy+26);
    // Bench inside
    pHRect(85,cy+60,70,22,6,"#2a1810");
    // Towel rack
    pHRect(46,cy+40,40,4,2,"#aaa");
    pHRect(46,cy+20,3,24,2,"#fff");
  }

  // Exit door
  pHRect(590,640,100,70,12,P.MBROWN);
  pHStroke(590,640,100,70,12,"#d8c070",4);
  pCirc(605,675,3,"#d0b860");
  pxCtx.fillStyle="#d8c070";pxCtx.font="bold 12px monospace";pxCtx.textAlign="center";
  pxCtx.fillText("LOBBY",640,683);
}
...
function drawFloor24() {
  ...
  // Plush carpet
  pxCtx.save();pxCtx.globalAlpha=0.06;
  for(let r=0;r<44;r++)for(let c=0;c<80;c++)if((r+c)%3!==0)pRect(c*16,r*17,16,17,"#c0a8d0");
  pxCtx.restore();
  // Crown molding
  pHRect(0,120,PX_W,8,4,"rgba(255,255,255,0.04)");

  // Chandelier
  const chX=640,chY=124;
  pRect(chX-2,chY,4,16,"#d0c080");
  pHRect(chX-36,chY+16,72,10,6,"#c0b060");
  for(let a=0;a<5;a++){const ax=chX-28+a*14; pRect(ax,chY+26,2,20,"#c0a840");}
  for(let a=0;a<5;a++){pCirc(chX-28+a*14,chY+52,6,"#e8d080");pCirc(chX-28+a*14,chY+52,8,"rgba(255,240,160,0.3)");}
  pxCtx.save();pxCtx.globalAlpha=0.05;
  for(let g=0;g<3;g++)pCirc(chX,chY+80+g*16,36,px(255,240,160)); pxCtx.restore();

  // Wall paintings
  pHRect(40,160,100,72,8,"#1a1020");pHStroke(40,160,100,72,8,"#806040",4);
  pHRect(50,170,80,20,4,"#2a1840");pRect(50,170,80,2,"#604080");
  pHRect(50,210,80,14,4,"#1a1020");
  pRect(50,140,4,28,"#806040");pRect(136,140,4,28,"#806040");
  pHRect(1140,160,100,72,8,"#1a1020");pHStroke(1140,160,100,72,8,"#806040",4);
  pHRect(1150,170,80,20,4,"#2a1840");pRect(1150,170,80,2,"#604080");
  pHRect(1150,210,80,14,4,"#1a1020");
  pRect(1150,140,4,28,"#806040");pRect(1236,140,4,28,"#806040");

  // Balcony
  pHRect(30,24,1220,96,12,"#080612");pHStroke(30,24,1220,96,12,"#9080b8",4);
  for(let p=0;p<12;p++){const gx=48+p*100;
    pHRect(gx,36,48,68,6,"rgba(100,80,140,0.18)");pHStroke(gx,36,48,68,6,"rgba(120,100,160,0.4)",2);
    pxCtx.save();pxCtx.globalAlpha=0.12;pRect(gx+4,44,6,50,"#ffffff");pxCtx.restore();}
  pxCtx.save();pxCtx.globalAlpha=0.06;
  for(let t=0;t<40;t++)pRect(t*32,76,32,48,"#9080a0");
  pxCtx.restore();
  pxCtx.save();pxCtx.globalAlpha=0.1;pxCtx.fillStyle="#201830";
  for(let b=0;b<16;b++){const bh=16+Math.sin(b*1.8)*16;pxCtx.fillRect(48+b*76,76-bh,44,bh);}
  pxCtx.restore();

  // King bed
  pHRect(140,160,420,280,24,"#100820");pHStroke(140,160,420,280,24,"#b0a0d0",3);
  // Headboard
  pHRect(140,130,420,40,14,"#201040");pHStroke(140,130,420,40,14,"#c0b0e0",2);
  pxCtx.save();pxCtx.globalAlpha=0.2;
  for(let hb=0;hb<8;hb++){pCirc(180+hb*52,148,12,"#6040a0");pCirc(180+hb*52,148,4,"#8060c0");}
  pxCtx.restore();
  // Bedside tables + lamps
  for(let bt of[{x:120,y:200},{x:520,y:200}]){
    pHRect(bt.x,bt.y,32,52,8,"#0e0818");
    pRect(bt.x+14,bt.y-12,4,20,"#664020");pCirc(bt.x+16,bt.y-16,10,"rgba(255,200,120,0.4)");
    pSCirc(bt.x+16,bt.y-16,12,"rgba(255,180,100,0.25)",2);
    if(bt.x===120){pHRect(bt.x+4,bt.y+4,16,12,4,"#302020");pCirc(bt.x+12,bt.y+10,4,"#fff");}}
  // Rug
  pHRect(100,400,460,290,40,"rgba(80,40,60,0.25)");pHStroke(100,400,460,290,40,"rgba(120,80,100,0.2)",3);
  pxCtx.save();pxCtx.globalAlpha=0.08;
  for(let rp=0;rp<3;rp++)pHRect(160+rp*120,410,60,260,16,"#604060");
  pxCtx.restore();
  // Pillows
  for(let pi=0;pi<3;pi++){pHRect(168+pi*130,178,100,56,14,"#e8e0f0");pHStroke(168+pi*130,178,100,56,14,"#c0b0d0",2);}
  // Duvet
  pHRect(160,250,340,176,16,"#2a1850");pHStroke(160,250,340,176,16,"#9078c0",2);
  pxCtx.save();pxCtx.globalAlpha=0.18;pRect(160,310,340,6,"#402870");pxCtx.restore();

  // Couch
  pHRect(80,520,340,110,18,"#1a0e30");pHStroke(80,520,340,110,18,"#a090c0",3);
  pHRect(95,540,90,70,12,"#301860");pHRect(200,540,90,70,12,"#301860");pHRect(305,540,90,70,12,"#301860");
  pHRect(85,520,330,20,6,"#281050");
  pHRect(80,520,20,110,8,"#1a0e30");pHRect(400,520,20,110,8,"#1a0e30");

  // Bathroom
  pHRect(920,140,310,410,18,"#08060e");pHStroke(920,140,310,410,18,"#9088a8",4);
  pHRect(940,450,50,90,8,"#100c18");pHStroke(940,450,50,90,8,"#a098b8",2);
  pCirc(975,495,3,"#c0b8a0");
  // Shower
  pHRect(1040,160,90,140,12,"#04060a");pHStroke(1040,160,90,140,12,"#7088b0",3);
  pCirc(1085,172,6,"#aaa");
  pxCtx.save();pxCtx.globalAlpha=0.08;pxCtx.strokeStyle="#88c8ff";pxCtx.lineWidth=1;
  for(let d=0;d<5;d++){pxCtx.beginPath();pxCtx.moveTo(1085,180);pxCtx.lineTo(1080+d*2,210+d*7);pxCtx.stroke();}
  pxCtx.restore();
  pHStroke(1130,160,8,140,4,"rgba(120,180,240,0.3)",3);
  // Double sink + mirror
  pHRect(960,220,60,16,6,"#fff");pHRect(1030,220,60,16,6,"#fff");
  pHRect(950,160,150,55,8,"rgba(160,180,220,0.3)");pHStroke(950,160,150,55,8,"rgba(160,170,210,0.5)",2);
  // Toilet
  pHRect(1060,350,60,80,14,"#fff");pHRect(1050,380,10,50,5,"#eee");
  pxCtx.strokeStyle="#ddd";pxCtx.lineWidth=2;pxCtx.beginPath();
  pxCtx.ellipse(1090,370,22,8,0,0,Math.PI*2);pxCtx.stroke();
  // Towel rack
  pxCtx.strokeStyle="#bbb";pxCtx.lineWidth=2;
  for(let tr=0;tr<3;tr++){pxCtx.beginPath();pxCtx.moveTo(1180,200+tr*20);pxCtx.lineTo(1210,200+tr*20);pxCtx.stroke();}
  pxCtx.save();pxCtx.globalAlpha=0.3;pHRect(1182,197,26,26,4,"#e8e0e0");pxCtx.restore();

  // Exit
  pHRect(590,640,100,70,12,P.MBROWN);pHStroke(590,640,100,70,12,"#d8c070",4);
  pCirc(605,675,3,"#d0b860");
  pxCtx.fillStyle="#d8c070";pxCtx.font="bold 12px monospace";pxCtx.textAlign="center";
  pxCtx.fillText("LOBBY",640,683);
}
...
function drawPixelPlayer() {
  ...
  const cx = px + ps/2, s = ps/8;
  const hR = 6*s, hCY = py + hR + s;
  const tW = 8*s, tH = 12*s, tY = hCY + hR + 2*s;
  // Shadow
  pCirc(cx, tY+tH+4, ps*0.6, "rgba(0,0,0,0.25)");
  // Legs
  pHRect(cx-tW/3-1, tY+tH, 2.5*s, 8*s, 2, "#1e2a42");
  pHRect(cx+tW/3-1, tY+tH, 2.5*s, 8*s, 2, "#1e2a42");
  // Arms
  pHRect(cx-tW/2-2.5*s, tY+2, 2.5*s, 7*s, 2, "#f0dcc0");
  pHRect(cx+tW/2, tY+2, 2.5*s, 7*s, 2, "#f0dcc0");
  // Torso
  pHRect(cx-tW/2, tY, tW, tH, 3, "#f4f7ff");
  pHStroke(cx-tW/2, tY, tW, tH, 3, "#d0d8e0", 1);
  // Head
  pCirc(cx, hCY, hR, "#f0dcc0");
  // Eyes
  pRect(cx-hR*0.4, hCY+1, 2, 2, "#1a1220");
  pRect(cx+hR*0.2, hCY+1, 2, 2, "#1a1220");
  // Mouth
  pRect(cx-2, hCY+hR*0.5, 4, 1, "#c0a080");
}
